...
public class WarehouseServiceImpl implements WarehouseService{
    ...
    public ResponseHolder<Collection<WarehouseDTO>> list() {
        ...
        if (entities.isEmpty())
            return new ResponseHolder<>(HttpStatus.NOT_FOUND,new ResponseError("1111","No data"));
    ...
    public ResponseHolder<WarehouseDTO> create(WarehouseDTO warehouseDTO) {
        ...
        if(operationRepository.hasExistSameWarehouseCode(warehouseDTO.getCode()))
            return new ResponseHolder<>(HttpStatus.NO_CONTENT,new ResponseError("2222","This warehouse is already created"));
        if (warehouseEntity.getName().isEmpty())
            return new ResponseHolder<>(HttpStatus.NO_CONTENT,new ResponseError("3333","Please Enter the warehouse name"));
